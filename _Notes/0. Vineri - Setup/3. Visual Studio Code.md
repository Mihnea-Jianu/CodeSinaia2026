Depends on [2. Python](./2.%20Python.md), [2. GitHub & Git](./1.%20GitHub%20&%20Git.md)  
Resources:  [Visual Studio Code](https://apps.microsoft.com/detail/xp9khm4bk9fz7q?hl=en-US&gl=US), [CodeSinaia-2026](https://github.com/inproted/CodeSinaia-2026)

Visual Studio Code (VSCode) is a powerful free Integrated Development Environment (IDE) used in academia and in industry. It is lightweight and extensible, allowing coding in a large set of programming languages (Python, C, C++, Java, C#, ...) by means of specific extension modules.
Steps to install VSCode:

<table>
    <thead><tr><th>OS</th><th>Steps</th></tr>
    </thead>
    <tbody>
    <tr><td valign="top">Windows</td><td><ol>
        <li>Click on <i>Start</i> button and type "Store". Click on the <i>Microsoft Store</i> app that shows up.</li>
        <li>In <i>Microsoft Store</i> search for "visual studio code" and select the "Visual Studio Code" app that shows up to the top, then click <i>Install</i>.<br><u>Note:</u> Do <b>not</b> install "Visual Studio Code - Insiders". That is a pre-release version: more features but less stable.</li>
    </ol></td></tr>
    <tr><td valign="top">Mac OS</td><td><ol>
        <li>TODO - Add content</li>
    </ol></td></tr>
    <tr><td valign="top">Linux</td><td><ol>
        <li>TODO - Add content</li>
    </ol></td></tr></tbody>
</table>

***
**VSCode Extensions**:
- [ ] Once installed, open VSCode. On the left side of the window, the 5th Icon from the top is the "*Extensions*" pane. Click on it and search for "*CodeSinaia 2026*" There should be one extension only showing up:  
    ==// TODO - Add screen capture==
- [ ] Alternatively, the following extensions can be installed manually, one by one:  
	==// TODO - Add list of extensions ==

----
**CodeSinaia-2026** repo setup:
- [ ] In VSCode click on "*File*" menu, then on "*Open Folder*".
- [ ] In the Open Folder window, navigate to the location of your local repo (i.e. `C:\Users\Florin\git\CodeSinaia-2026`), then click "*Select Folder"*.
- [ ] A dialog may pop up asking you to Acknowledge the folder is trusted. Please do so.

If all went well, you'll notice in the *Explorer* pane (first button from the left bar) the content of your local **CodeSinaia_2026** repository.
- [ ] From the *Explorer* pane, click & open the "*HelloPython.py*". Run it by clicking the *Run* button on the top-right side of the edit window.
   You should see a terminal window with an output like the following:
	![[Screenshot 2026-05-24 164543.png]]
- [ ] Open next the "*HelloJupyter.ipynb*" file. Run it by clicking on the "Run All" button on top of the edit window.
   You should see a popup asking for the Jupyter kernel to be installed. Please acknowledge and let the installation process to complete (less than a minute). When installation completes, you should see the output of the code block generated right underneath it:
    ![[Screenshot 2026-05-24 164918.png]]
----
**Copilot:**
- [ ] In VSCode, click on the account icon (lower on the left bar, above the settings button) and Login with your GitHub account.
- [ ] On the top bar, next to the search bar, click on the *Copilot* icon and follow the instructions to enable Copilot on your GitHub account.
- [ ] Open the Copilot chat window and type a "Hello there ..." prompt to verify it is setup correctly

